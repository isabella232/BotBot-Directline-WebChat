...
import { Activity, sendMessage } from './Store';
...
export class HistoryView extends React.Component<HistoryProps, {}> {
  ...
  componentWillUpdate() {
    this.scrollToBottom =
      Math.abs(this.scrollMe.scrollHeight - this.scrollMe.scrollTop - this.scrollMe.offsetHeight) <=
      1;
  }
  ...
  componentDidUpdate() {
    if (this.props.format.carouselMargin == undefined) {
      ...
      const maxContentWidth =
        (this.carouselActivity.messageDiv.offsetParent as HTMLElement).offsetWidth - paddedWidth;
